...
rule copy : ### Permet de copier les génomes
	input: 
		"/media/newvol/yascimkamel/Pipeline/genome/copie/{BD}_f.fasta"
	output:
                expand("/media/newvol/yascimkamel/Pipeline/Snakemake/copie/{BD}_copied.fasta", espece=ESPECE, BD=liste_BD)
	shell : 
                "cp {input} {output}"
